...
log = logging.getLogger("bk_gpt_feed")
...
PROMPT_VERSION = "bk_narrative_v1.0"
...
def build_market_snapshot(engine):
    ...
    latest_ts = snap["ts"].max() if not snap.empty else None
...
def safe_openai_call(system_prompt, user_prompt, max_retries=3, backoff_seconds=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_error = e
            log.warning(f"[BK] OpenAI error attempt {attempt+1}: {e}")
            time.sleep(backoff_seconds * (attempt + 1))
    raise RuntimeError(f"OpenAI bk narrative call failed after retries: {last_error}")
...
def run_gpt_feed():
    ...
    try:
        ...
        log.info(
            f"[BK] {ts} fp={fingerprint} saved "
            f"{len(narratives_rows)} narratives, {len(nar_assets_rows)} links"
        )
    except IntegrityError:
        log.info(f"[BK] {ts} fp={fingerprint} duplicate, skipping inserts")
